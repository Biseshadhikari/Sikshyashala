...
def course(request, slug):
    ...
    try:
        lesson = lessons.objects.get(
            lesson_number=lesson_number, Course=courses)
...
def signin(request):
    if not request.user.is_authenticated:
        if request.method == 'POST':
            ...
            if user_obj is None:
                messages.success(
                    request, 'User not found!!! please signin with correct username')
                return redirect('/login')
    ...
    try:
        ...
        if profile_obj:
            if profile_obj.is_verified:
                messages.success(
                    request, 'Your account is already verified! Please Sign in')
                return redirect('/login')
